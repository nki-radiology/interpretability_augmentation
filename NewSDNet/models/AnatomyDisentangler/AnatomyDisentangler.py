...
class CentresClassifier(nn.Module):
    def __init__(self, n_anatomy_channels, num_centres):
        super(CentresClassifier, self).__init__()
        self.n_anatomy_channels = n_anatomy_channels
        self.num_centres = num_centres
        self.classifier = resnet50(weights=None)
        self.classifier.conv1 = nn.Conv2d(
            self.n_anatomy_channels, 64, kernel_size=7, stride=2, padding=3, bias=False
        )
        self.linear_size = list(self.classifier.children())[-1].in_features
        self.classifier.fc = nn.Linear(self.linear_size, self.num_centres)

    def forward(self, anatomy_dis):
        # print(f"\nANATOMY IN INPUT AL CLASSIFIER SHAPE: {anatomy_dis.shape}")
        cls = self.classifier(anatomy_dis)
        pred = F.softmax(cls, dim=1)

        return pred


class SDNetAnatomyDisentangler(nn.Module):
    def __init__(
        self,
        width,
        height,
        ...
        super(SDNetAnatomyDisentangler, self).__init__()
        """
        Args:
            width: input width
            height: input height
        ...
        self.segmentor = Segmentor(8, self.num_classes)
        self.decoder = Decoder(
            self.anatomy_out_channels, self.z_length, self.num_mask_channels
        )
        self.classifier = CentresClassifier(
            n_anatomy_channels=3, num_centres=self.num_centres
        )
        self.adversarial_classifier = CentresClassifier(
            n_anatomy_channels=5, num_centres=self.num_centres
        )

    def forward(self, x, script_type):
        a_out = self.a_encoder(x)
        seg_pred = self.segmentor(
            a_out[:, :, :, :]
        )  # might be too strong; I also tried passing in the entire anatomy, let's see how it goes lol; I have chose channel 4-8 because the polyp is usually in the last channels
        z_out, mu_out, logvar_out = self.m_encoder(x)
        if script_type == "training":
            cls = self.classifier(a_out[:, :3, :, :])
            cls_adv = self.adversarial_classifier(a_out[:, 3:, :, :])
