...
class AEncoder(nn.Module):
    ...
    def forward(self, x):
        out = self.unet(x)
        out = F.gumbel_softmax(out, hard=True, dim=1)  # hard=False
...
class MEncoder(nn.Module):
    def __init__(self, z_length):
        ...
        self.activ = nn.LeakyReLU(0.03, inplace=False)
...
class SDNet(nn.Module):
    def __init__(
        ...
        self.segmentor = Segmentor(3, 1)
        self.decoder = Decoder(8, 8, self.num_mask_channels)

    def forward(self, x, script_type, saliency=None):
        a_out = self.a_encoder(x)
        a_out_anatomy = a_out[:, 3:6, :, :]
        if saliency is not None:
            # print(f"\nSHAPE DI A_OUT_ANATOMY: {a_out_anatomy.shape}")
            saliency_3_ch = saliency[:, 3:6, :, :]
            # print(f"\nSHAPE DI SALIENCY_3_CH: {saliency_3_ch.shape}")
            if random.choice([0, 1]) == 1:
                # print(f"\nSONO NEL IF SALIENCY / IF RANDOM.CHOICE = 1")
                # print(f"\nSONO NEL NOT NON DEL FORWARD DELLE SALIENCY!!!\n")
                input_to_segmentor = (
                    a_out_anatomy * saliency_3_ch
                )  # this is a change w.r.t original architecture
                # print(f"\nSHAPE DI INPUT TO SEGMENTOR: {input_to_segmentor.shape}")
                # print(
                #     f"\nGUARDO SE A_OUT E INPUT TO SEGMENTOR SONO DIVERSI: {torch.unique(a_out_anatomy[a_out_anatomy != input_to_segmentor], return_counts=True)}"
                # )
            else:
                # print(f"\nSONO NEL IF SALIENCY / IF RANDOM.CHOICE = 0")
                input_to_segmentor = a_out_anatomy
        else:
            input_to_segmentor = a_out_anatomy
        # print(f"SHAPE OF INPUT TO SEGMENTOR: {input_to_segmentor.shape}")
        # dummy assignment for when we don't have saliency maps
        # input_to_segmentor = a_out * saliency
        seg_pred = self.segmentor(input_to_segmentor)
