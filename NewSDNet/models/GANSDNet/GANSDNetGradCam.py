...
class GANSDNetGradCamLightning(pl.LightningModule):
    def __init__(
        self,
        lr: float,
        ...
        self.anatomy_train = []
        self.input_to_segmentor = []
        self.input_to_segmentor_columns = ["channel 4", "channel 5", "channel 6"]
    ...
    def training_step(self, batch: tuple[torch.Tensor, torch.Tensor], batch_idx):
        imgs, labels, centres, saliency = batch
        ...
        (
            ...
            logvar_out,
            input_to_segmentor,
        ) = self.sdnet(imgs, script_type="training", saliency=saliency)
        ...
        if self.current_epoch % 30:
            if batch_idx == 5:
                ...
                inp_to_seg = [
                    [inp[0, ...], inp[1, ...], inp[2, ...]]
                    for inp in input_to_segmentor
                ]
                self.input_to_segmentor.extend(inp_to_seg)
    ...
    def on_train_end(self) -> None:
        ...
        inpu_to_seg_wandb = []
        for row in self.input_to_segmentor:
            anatomy_wand = [
                wandb.Image(row[0]),
                wandb.Image(row[1]),
                wandb.Image(row[2]),
            ]
            inpu_to_seg_wandb.append(anatomy_wand)
        ...
        self.trainer.logger.log_table(
            key="input to segmentor",
            columns=self.input_to_segmentor_columns,
            data=inpu_to_seg_wandb,
        )
        self.trainer.logger.log_table(
